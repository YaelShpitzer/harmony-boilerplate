...
import {ProductState, TypesNames, ActionCreator, SetProductsAction, SetFilterProductAction, SetProductAction, Product, DeleteProductAction
} from './interfaces';
...
const getFilterTextById = (state: ApplicationState) => state.product.filter.filterTextById;

const getProductsList = (products: Product[], inStockOnly: boolean, filterText: string,filterTextById:string) => {
	return products.filter((product: Product) => {
		if (!isProductContainsText(product, filterText)) return false;
		if (inStockOnly && !product.isInStock) return false;
		if (filterTextById&&product.id!=filterTextById) return false;
...
const getProductsSelector = createSelector(
	[getProducts, getIsInStock, getFilterText,getFilterTextById],
	getProductsList
);
...
const deleteProductReducer = (state: ProductState,action:DeleteProductAction) => {
	...
	return from(state).merge({ products });
